...
import GraphLayouter from './GraphLayouter.js';
...
function openLog(file, callback) {
  // check file api availability
  if (!window.FileReader) {
    return window.alert(
      'Looks like you use an older browser that does not support drag and drop. ' +
  ...
  reader.onload = async function (e) {
    ...
    const graphLoader = new GraphLoader(model);
    const nodesAndEdges = graphLoader.LoadGraph();
    const layouter = new GraphLayouter(nodesAndEdges);

    const xml = await layouter.LayoutGraph()  
    callback(xml);
  };
...
var logInput = $('<input type="file" />')
  ...
  .on('change', function(e) {
    openLog(e.target.files[0], openBoard);
});
